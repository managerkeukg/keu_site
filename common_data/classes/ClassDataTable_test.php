...
{ 	// actions
	...
	*/
	...
	{ 
		...
		if (isset($this->foreign_key_array) AND !empty($this->foreign_key_array))
		{
			$this->get_foreign_key_field_ids ($data);
			$this->get_foreign_key_field_array ();
		}
		////echo "<PRE> Header "; print_r($header); echo "</PRE>";
		////echo "<PRE>"; print_r($tr_javascript); echo "</PRE>";
		if ($view_type=="table") 
		{
			...
			///*
				...
				$object_table->set_data($data);
				
				//$object_table->set_th_array (array("No записи", "Учебный год", "Полугодие", "Группа", "Предметы", "Браузер", "Время	старта", "Окончание", "IP" ));
				$object_table->set_th_array ($header);
				
				////$object_table->column_value_array_foreign ("year", $array_years , array("name"));
				////$object_table->column_value_array_foreign ("group", $array_groups , array("name"));
				if ($this->user_module_permissions['1']=='1' OR ($this->user_module_permissions['2']=='2') ) 
				{
					$object_table->data_key_add ("view");
					$view_link="
						<a href=\"".$this->url."&".$this->action_page."=".$this->current_page."&".$this->action_name."=".$this->action_view."&".$this->id_parameter."={{id}} \" >
							<img src=\"".$this->data_path."images/next.png\" border=\"0\" ></img>
						</a>";
					$object_table->data_key_value ("view", $view_link);
					$object_table -> header_add ("view", "");
				}
				
				if (isset($this->status_field) AND !empty($this->status_field)) 
				{
					///echo "<pre>USER MODULE PERMISSIONS "; print_r($this->user_module_permissions) ; echo "</pre>";
					if ($this->user_module_permissions['1']=='1' OR ($this->user_module_permissions['3']=='3') ) 
					{ 	
						/*
						$object_table->data_key_add ("show_hide");
						$show_hide_link="
						<a href=\"".$this->url."&".$this->action_page."=".$this->current_page."&".$this->action_name."=".$show."&".$this->id_parameter."={{id}} \" >
							<img src=\"".$this->data_path."images/".$show_image."\" border=\"0\" ></img>
						</a>";
						$object_table->data_key_value ("show_hide", $show_hide_link);
						$object_table -> header_add ("show_hide", "");
						*/
						$show_hide_link="
						<a href=\"".$this->url."&".$this->action_page."=".$this->current_page."&".$this->action_name."={{hidesss}}&".$this->id_parameter."=
						{{id}}
						\" >
							<img src=\"".$this->data_path."images/";
						$object_table->data_key_add ("show_hide");
						//$object_table->data_key_code ("show_hide", "http://localhost/SITE_CLASS/admin/profiles/show_hide.php");
						$object_table->data_key_code_status ("show_hide", "show_hide.php", $this->status_field, $show_hide_link);
						$object_table -> header_add ("show_hide", "");
					}   
				} 
				else {
				...
				{ 	
					$object_table->data_key_add ("edit");
					$edit_link="
						<a href=\"".$this->url."&".$this->action_page."=".$this->current_page."&".$this->action_name."=".$this->action_edit."&".$this->id_parameter."={{id}} \" >
							<img src=\"".$this->data_path."images/edit.gif\" border=\"0\" ></img>
						</a>";
					$object_table->data_key_value ("edit", $edit_link);
					$object_table -> header_add ("edit", "");			
				} 				
				if ($this->user_module_permissions['1']=='1' OR ($this->user_module_permissions['6']=='6') ) 
				{ 
					$object_table->data_key_add ("delete");
					$delete_link="
						<a href=\"".$this->url."&".$this->action_page."=".$this->current_page."&".$this->action_name."=".$this->action_delete."&".$this->id_parameter."={{id}} \" onclick=\"return confirm('Вы уверены, что хотите удалить?');\">
							<img src=\"".$this->data_path."images/delete.gif\" border=\"0\" ></img>
						</a>";
					$object_table->data_key_value ("delete", $delete_link);
					$object_table -> header_add ("delete", "");
				}
					
				//$object_table->data_key_delete ("status");
				
				/*
				$object_table->data_key_add ("fio");
				$object_table->data_key_value ("fio", "surname text index.php?id={{id}}&profile={{profile}}");
				$object_table -> header_add ("fio", "ФИО");
				
				$object_table->column_value ("status", "ok");
				$object_table->column_delete ("status");
				$object_table->column_add ("address", "Адрес");
				$object_table->column_add ("address2", "Адрес2");
				$object_table->column_value ("address", "ok id={{id}}&profile={{profile}}");
				$object_table->column_value ("address2", "some text{{id}}");
				*/
				$object_table->column_delete ($this->status_field);
				$object_table->css_file="css/html_table_blue.css";
				if (isset($this->foreign_key_array) AND !empty($this->foreign_key_array) AND is_array($this->foreign_key_array))
				{
					foreach ($this->foreign_key_array as $key => $value )
					{
						////echo "<PRE> data  "; print_r($this->array_foreign_values[$key]); echo "</PRE>";
						$object_table->column_value_array_foreign ($key, $this->array_foreign_values[$key], array($value['field']));
					}
				}
				
				if (isset($this->array_foreign_key) AND !empty($this->array_foreign_key) AND is_array($this->array_foreign_key))
				{
					foreach ($this->array_foreign_key as $key => $value )
					{
						////echo "<PRE> data foreign "; print_r($value['data']); echo "</PRE>";
						$object_table->column_value_array_foreign ($key, $value['data'], array($value['showfield']));
					}
				}
		...
		if ($this->user_module_permissions['1']=='1' OR ($this->user_module_permissions['2']=='2') )
		{
			...
			if($cat) 
			{
							...
							foreach ($this->image_fields_array as $field => $data) {
								if(isset($record_array[$field])) { 
									...
									{	
										...
										$record_array[$field]="<img src=\"".$this->data_path."/images/general/nophoto.jpg\" width=\"".$data['width']."\"> </img>";
									}
								}
							}// end foreach
						} //if
				...
				if (!empty($record_array)) {
					$this->record_array=$record_array;
	...
	{  
		...
		////echo "<pre> foreign_values "; print_r($this->array_foreign_key); echo "</pre>";
		foreach ($array as $row)
		{ 
			if($row['Key']=="PRI" OR $row['Field']==$this->status_field) {}
			else 
			{   
				...
				if (isset($this->array_foreign_key[$field]) AND !empty($this->array_foreign_key[$field]) AND is_array($this->array_foreign_key[$field]))
				{
					//echo $this->array_foreign_key[$field]['showfield'];
					////echo "<pre> foreign_values "; print_r($this->array_foreign_key[$field]['showfield']); echo "</pre>";
					$field_show=$this->array_foreign_key[$field]['showfield'];
					$record_id=$record_array[$this->auto_increment];
					$column_value=$this->array_foreign_key[$field]['data'][$column_value][$field_show];
					//echo $record_id; // id record
				}
			...
			{  
				...
				{ 
					...
					{ 
						...
						////echo "<pre> foreign_field_array "; print_r ($foreign_field_array); echo "</pre>";
					...
					if (isset($this->array_foreign_key[$row['Field']]) AND !empty($this->array_foreign_key[$row['Field']]))
					{
						////echo "<pre> array_field_foreign "; print_r ($this->array_foreign_key[$row['Field']]['data']); echo "</pre>";
						$array_select=$this->array_convert ($this->array_foreign_key[$row['Field']]['data'], $this->array_foreign_key[$row['Field']]['showfield']);
						////echo "<pre> array_select "; print_r ($array_select); echo "</pre>";
						$input_text=$this->create_selectbox ( $array_select, $field, $field, $record_array[$field]);
					}	
	    ...
	    { 
			...
			else 
			{
				...
				if (isset($this->array_foreign_key[$row['Field']]) AND !empty($this->array_foreign_key[$row['Field']]))
				{
					////echo "<pre> array_field_foreign "; print_r ($this->array_foreign_key[$row['Field']]['data']); echo "</pre>";
					$array_select=$this->array_convert ($this->array_foreign_key[$row['Field']]['data'], $this->array_foreign_key[$row['Field']]['showfield']);
					$input_text=$this->create_selectbox ($array_select, $field, $field, $this->binded_field_value[$field]);
				}	
...
<script type="text/javascript">
	...
	//----- start foreign functions ----
	private $foreign_key_field_ids = array();
	private function get_foreign_key_field_ids ($data)
	{
		////echo "<PRE> foreign_key_array "; print_r($this->foreign_key_array); echo "</PRE>";
		foreach ($data as $key => $value)
		{
			foreach ($value as $key1 => $value1)
			{
				if (isset($this->foreign_key_array[$key1]))
				{
					$this->foreign_key_field_ids[$key1][$value[$key1]]=$value[$key1];
				}
			}
		}
		////echo "<PRE> foreign_key_field_ids "; print_r($this->foreign_key_field_ids); echo "</PRE>";
	}
	
	private $array_foreign_values = array();
	private function get_foreign_key_field_array ()
	{
		if (isset($this->foreign_key_field_ids) AND !empty($this->foreign_key_field_ids))
		{
			require_once _COMMON_DATA_PATH_."classes/ClassTableQuery.php";
			foreach ($this->foreign_key_array as $key => $value) 
			{
				$object_query=new TableQuery;
				$object_query->order_by_field=$value['key'];
				$where_text="";
				$i=0; $or_text="";
				foreach ($this->foreign_key_field_ids[$key] as $value_ids)
				{
					$i++; 
					if ($i==1) {$or_text="";} else {$or_text=" OR ";}
					$where_text=$where_text.$or_text." (`".$value['key']."` = '".$value_ids."') ";
				}
				$query_foreign="";
				$query_foreign= "SELECT `".$value['key']."`, `".$value['field']."` FROM `".$value['table']."` WHERE ".$where_text."";
				////echo "<br>".$query_foreign;
				$array_foreign_key_field_values = $object_query->query ($query_foreign);
				if (isset($array_foreign_key_field_values) AND !empty($array_foreign_key_field_values) AND is_array($array_foreign_key_field_values))
				{
					////echo "<pre> array_foreign_key_field_values "; print_r($array_foreign_key_field_values); echo "</pre>";
					$this->array_foreign_values[$key]=$array_foreign_key_field_values;
				}
				else {
					
				}
			}
			////echo "<pre> array_foreign_values "; print_r($this->array_foreign_values); echo "</pre>";
		}
	}
	
	private $array_foreign_key = array();
	public function foreign_key_array ($field, $array_foreign, $field_to_show)
	{
		$this->array_foreign_key[$field]['data'] = $array_foreign;
		$this->array_foreign_key[$field]['showfield'] = $field_to_show;
		////echo "<pre> foreign_values "; print_r($this->array_foreign_key); echo "</pre>";
	}
	
	private $foreign_key_array = array() ;
	public function foreign_key ($field, $foreign_table, $foreign_field, $foreign_field_id)
	{
		...
		////echo "<pre>foreign_key "; print_r($this->foreign_key_array); echo "</pre>";
	}
	...
	{ 
		...
		$array_foreign = $object_query->query ("SELECT `".$foreign_field."` FROM `".$foreign_table."` WHERE `".$foreign_field_id."` = '".$id."'");
		if (isset($array_foreign) AND !empty($array_foreign) AND is_array($array_foreign))
		{
			////echo "<pre> foreign "; print_r($array_foreign); echo "</pre>";
			return $array_foreign[0][$foreign_field];
        }
		else {
			return "";
		}
	...
	{ 
		...
		if(mysql_num_rows($rez) > 0)
		{
			...
			////echo "<pre>"; print_r ($table_array); echo "</pre>";
			return $table_array;
		} else {}
	}
	...
	private function create_selectbox ($array_select, $select_tag_id, $select_tag_name, $option_value_selected)
	{   
		require_once _COMMON_DATA_PATH_."classes/ClassSelectHtml.php";
		$object_select = new SelectHtml5;
		$object_select->id = $select_tag_id;
		$object_select->name = $select_tag_name;
		$object_select -> array_options = $array_select;
		$object_select -> selected_value = $option_value_selected;
		return $object_select->display ();
	}
	...
	private function array_convert ($data_array, $value_field)
	{   
		foreach ($data_array as $key => $value)
		{
			$array_new[$key] = $value[$value_field];
		}
		return $array_new;
	}
} // end of class
