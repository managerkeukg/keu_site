...
$data_array=$object-> query("SELECT * FROM `keu_career` where `status`='1'  ORDER BY `id`");
//echo "<pre>"; print_r($data_array); echo "</pre>";
if (isset($data_array) AND !empty ($data_array)) {
    ?>
	 <h2 style="color:black; padding-left:100px;"><?php echo $array_frontpage_xml['frontpage_career_name_'.$lang_current]; ?></h2>
	...
	{ $i++;
	  $link_url="";   if (empty($value['link'])) { $link_url="index.php?show=205&id=".$value['id']; } 
              else { $link_url=$value['link']." \" target=\"_blank ";}
	  ?>
       <DIV class="cols">
						<div class="view <?php echo "view-".$effect_hover[$i]; ?>">
							<img src="images/career/<?php echo $value['photo']; ?>" />
							<div class="mask">
