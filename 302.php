...
if (isset($_POST) AND !empty($_POST))
{
    ...
    if (empty($error_text)) 
	{
    	$query = "INSERT INTO `keu_online_registration` (id, name, surname, patronymic, birthdate, sex, level, graduate_from, graduate_year, attestate, profile, citizenship, passport_data, mobile, teacher, email, rDate, status)
			VALUES (
				NULL,
				'".$name."',
				'".$surname."',
		...
		//$query = "select * from `keu_online_registration` order by `id` limit 1, 1;";
	...
	{ 
		...
		echo "Ошибка при введении данных - ".mysql_error(); 
	} 
	} else { echo "Ошибка при наборе данных - ".$error_text;}
}
?>