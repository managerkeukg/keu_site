...
$data_array=$object-> query("SELECT * FROM `keu_international` where `status`='1'  ORDER BY `id`"); // `id`, `name_ru`, `parent`
   ...
   foreach ($array as $key =>$value)
	{
	   if ($parent==$value['parent'])
		   { echo "<DIV class=\"sub_info\"><a href=\"index.php?show=204&id=".$value['id']."\">".$array_name[$value['id']]['name_'.$lang_current];
		  ...
		  <DIV style="width:300px; float:right;">
            <?php
				...
				foreach($parented_array['0'] as $key=>$value) {
					 echo "<DIV class=\"info\"><a href=\"index.php?show=204&id=".$key."\">".$new_array_id[$key]['name_'.$lang_current].""; ///echo "<li>$key";
